...
class CandidateGraph:
    ...
    def generate_candidate_graph(self, trajectory, candidate_roads, candidate_points, candidate_distance):
            ...
            for j, point_j in enumerate(candidate_points[i]):
                for k, point_k in enumerate(candidate_points[i + 1]):
                    if f"{i}&{j}" not in self.candidate_graph.nodes.keys():
                        observation_probability = self.observation_probability_func(point_j, trajectory[i], candidate_distance[i][j])
                    ...
                    if f"{i + 1}&{k}" not in self.candidate_graph.nodes.keys():
                        observation_probability = self.observation_probability_func(point_k, trajectory[i + 1],  candidate_distance[i+1][k])
...
class AIVMM:
    ...
    def gps_observation_probability(self, candidate_point, sample_point, candi_dist_cur):
        """
        GPS点的观测概率
    ...
    def create_candidate_graph(self, trajectory, candidate_roads, candidate_points, candidate_distance):
        ...
        # haoshi
        candidate_graph_obj.generate_candidate_graph(trajectory, candidate_roads, candidate_points, candidate_distance)
    ...
    def candidate_edge_voting(self, trajectory, candidate_roads, candidate_points, candidate_distance, is_show=True):
        n = len(trajectory)
        final_path = []
        lop_seq, f_value_seq = self.find_local_optimal_path_sequence(trajectory, candidate_roads, candidate_points, candidate_distance)
...
class Main:
    ...
    def match_candidate(self, trajectory, timestamp, is_show=False, rate=15):
        ...
        final_path = self.aivmm.candidate_edge_voting(trajectory, candidate_roads, candidate_points, candidate_distance)
