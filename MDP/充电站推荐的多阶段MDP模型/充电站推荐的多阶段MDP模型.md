...
# 总结与展望
CS推荐问题本质上是一个决策问题，所以我们求助于MDP来解决这个问题。然而，MDP的“维度诅咒”问题使其不适合CS建议。因此，我们将其建模为MMDP，通过引入PDS和IDS状态，对MDP的扩展。这种扩展的突出点在于，MDP的状态转换被分解为几个阶段，这大大降低了状态空间和转换复杂性，这也是使我们的命题优于其他机制的主要原因。提出的在线学习算法的好处是不需要关于所有概率分布的先验知识，这使得算法很好地适合CS推荐场景。对于我们今后的工作，以分布式的方式提出建议将是很有趣的。即，每个CS扮演推荐服务器的角色，负责其区域内的每个EV。此外，我们还将研究深度学习技术，并研究MMDP如何与深度学习一起工作，以提高CS推荐性能。
